...
ImgCropper.prototype = {
  ...
  SetPos: function() {
	...
	cropX = o.offsetLeft;
	cropY = o.offsetTop;
	cropWidth = o.offsetWidth;
	cropHeight = o.offsetHeight;
  },  
  //切割预览
  PreView: function() {
	//按比例设置宽度和高度
	...
	var o = this.Drag, w = this.viewWidth, h = w * o.offsetHeight / o.offsetWidth;
	var scale = w / o.offsetWidth;
	if(h > this.viewHeight) { h = this.viewHeight, w = h * o.offsetWidth / o.offsetHeight; scale = h / o.offsetHeight; }
	if(w > this.viewWidth)  { w = this.viewWidth, h = w * o.offsetHeight / o.offsetWidth; scale = w / o.offsetWidth;}
	//获取对应比例尺寸
...
if(parseInt(jsonFaceStr.face) == 1){
	var faceX = parseInt(jsonFaceStr.x);
	var faceY = parseInt(jsonFaceStr.y);
	var faceWidth = parseInt(jsonFaceStr.width);
	var faceHeight = parseInt(jsonFaceStr.height);
	var ic = new ImgCropper("bgDiv", "dragDiv", sourcePicShowUrl, maxWidth, maxHeight, {
		dragTop: faceY, dragLeft: faceX, dragWidth:faceWidth, dragHeight:faceHeight,
		Right: "rRight", Left: "rLeft", Up:	"rUp", Down: "rDown",
		RightDown: "rRightDown", LeftDown: "rLeftDown", RightUp: "rRightUp", LeftUp: "rLeftUp",
		View: "viewDiv", viewWidth: finalWidth, viewHeight: finalHeight, Scale: false
	})
	//document.getElementById("faceMsg").innerHTML="<font color=red>识别有人脸</font><br>位置：face:"+jsonFaceStr.face+",x:"+faceX+",y:"+faceY+",W:"+faceWidth+",H:"+faceHeight;
}
else{
	var ic = new ImgCropper("bgDiv", "dragDiv", sourcePicShowUrl, maxWidth, maxHeight, {
		dragTop: 100, dragLeft: 100, dragWidth:finalWidth, dragHeight:finalHeight,
		Right: "rRight", Left: "rLeft", Up:	"rUp", Down: "rDown",
		RightDown: "rRightDown", LeftDown: "rLeftDown", RightUp: "rRightUp", LeftUp: "rLeftUp",
		View: "viewDiv", viewWidth: finalWidth, viewHeight: finalHeight
	})
	//document.getElementById("faceMsg").innerHTML="<font color=red>识别无人脸</font><br>";
	document.getElementById("detecRes").innerHTML += "<span class='detecWarn'>经识别此照片有可能不是证件照</span>";
	//document.getElementById("btnupload").value="照片不符合要求";
...
Scale(true);
function doCrop(theForm){
	theForm.cropX.value = cropX;
	theForm.cropY.value = cropY;
	theForm.cropWidth.value = cropWidth;
	theForm.cropHeight.value = cropHeight;
	theForm.submit();
}
